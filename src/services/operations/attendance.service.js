        ...
        records.forEach((record) => {
            ...
            if (!userMap.has(userId)) {
                userMap.set(userId, {
                    ...
                    department_name:
                        (user.profile && user.profile.departmentInfo && user.profile.departmentInfo.name) || null,
            ...
            if (!userEntry.attendanceTypeMap.has(typeKey)) {
                userEntry.attendanceTypeMap.set(typeKey, {
                    ...
                    default_duration_minutes: record.attendanceType
                        ? record.attendanceType.default_duration_minutes
                        : null,
                    dailyMap: new Map(),
                });
            }
            ...
            if (
                ...
                (!dateEntry.latestCheckOut || record.check_out_time > dateEntry.latestCheckOut)
            ) {
                ...
                for (const [, dateEntry] of typeEntry.dailyMap.entries()) {
                    ...
                    if (dateEntry.earliest && dateEntry.latestCheckOut) {
                        durationMinutes = Math.round(
                            (new Date(dateEntry.latestCheckOut) - new Date(dateEntry.earliest)) / 60000,
                        );
                        duration = `${Math.floor(durationMinutes / 60)}h ${durationMinutes % 60}m`;
                    }
        ...
        attendances.forEach((attendance) => {
            ...
            if (!summaryMap[userId]) {
                summaryMap[userId] = {
                    ...
                    name: user.name,
                    department:
                        user.profile && user.profile.department ? user.profile.department : user.department || "N/A",
                    dates: {},
                };
            }
    ...
    try {
        const session = await db.AttendanceSession.findAll({
            ...
            include: [
                ...
                { model: db.AttendanceType, as: "attendance_type" },
            ],
        });
        return { success: true, data: session };
    } catch (error) {
