...
export const checkOutOfficeService = async (payload) => {
    ...
    try {
        ...
        // 8. Tạo thông báo
        await createOfficeCheckOutNotification(
            user_id,
            attendanceLocation,
            office_location_id_check_out,
            attendance_category
...
const findOpenAttendanceRecord = async (user_id, office_location_id, attendance_type_id) => {
    ...
    if (!session) {
        throw new Error("Bạn chưa có phiên chấm công vào tại văn phòng này");
    }
    ...
    if (!attendance) {
        throw new Error("Bạn chưa chấm công vào tại văn phòng này hoặc bản ghi chấm công không tồn tại");
    }
...
const validateCheckOutConditions = (attendance, payload) => {
    if (attendance.check_out_time) {
        throw new Error("Bạn đã thực hiện chấm công ra trước đó");
    }
...
const prepareCheckOutData = (attendance, payload, checkOutTime, attendanceLocation, officeLocationCheckOut) => {
    ...
    const locationNameCheckOut = payload.location_name_check_out
        ? String(payload.location_name_check_out).trim()
        : null;
