 ...
 * Kiểm tra cả session mở và bản ghi chấm công đã hoàn tất
 */
const checkExistingOfficeSession = async (user_id, office_location_id, attendance_type_id) => {
    ...
    // 1. Kiểm tra xem đã có session mở trong ngày hôm nay không
    ...
    if (openSession) {
        ...
        return {
            ...
            session: {
                id: openSession.id,
                attendanceLocation: openSession.attendanceLocation
                    ? {
                          id: openSession.attendanceLocation.id,
                          name: openSession.attendanceLocation.name,
                      }
                    : null,
    ...
    // 2. Kiểm tra xem đã có bản ghi chấm công đã check-out trong ngày không
    const completedAttendance = await db.Attendance.findOne({
        where: {
            user_id,
            office_location_id,
            status: "checked_out",
    ...
    if (completedAttendance) {
        ...
        return {
            ...
            message: checkOutAt
                ? `Bạn đã hoàn tất chấm công vào lúc ${checkOutAt.split("T")[1].substring(0, 5)}`
                : `Bạn đã hoàn tất chấm công tại vị trí này trong ngày`,
            lastAttendance: {
                id: completedAttendance.id,
                check_in_time: toVietnamTimeISO(completedAttendance.check_in_time),
                check_out_time: checkOutAt,
                location: completedAttendance.attendanceLocation
                    ? {
                          id: completedAttendance.attendanceLocation.id,
                          name: completedAttendance.attendanceLocation.name,
                      }
                    : null,
            },
        };
