...
export const assignRoleService = async (userId, roleId, assignedById) => {
    try {
        ...
        const assigner = await db.User.findOne({
        ...
        // if (
        //     !assigner ||
        //     !assigner.userRoles.some((r) => r.role.name === "admin")
        // ) {
...
export const revokeRoleService = async (userId, roleId, revokedById) => {
    try {
        ...
        const revoker = await db.User.findOne({
        ...
        if (
            !revoker ||
            !revoker.userRoles.some((r) => r.role.name === "admin")
        ) {
