...
/**
 * Migration 013: Tạo bảng Check In Types (Loại chấm công)
 *
...
export async function up(queryInterface, Sequelize) {
    ...
    await queryInterface.createTable("check_in_types", {
        id: {
  ...
  `);
    ...
    BEFORE INSERT OR UPDATE ON check_in_types
...
export async function down(queryInterface, Sequelize) {
    ...
    await queryInterface.sequelize.query(
        "DROP TRIGGER IF EXISTS trg_set_default_duration ON check_in_types;"
    );
    await queryInterface.sequelize.query(
        "DROP FUNCTION IF EXISTS set_check_in_type_default_duration();"
    );
    await queryInterface.dropTable("check_in_types");
}
