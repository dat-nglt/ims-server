...
/**
 * Model CheckInType (Loại chấm công)
 *
 * Các trường:
 * - id
...
export default (sequelize, DataTypes) => {
    const CheckInType = sequelize.define(
        "CheckInType",
        {
            id: {
                ...
                beforeSave: (instance) => {
                    try {
                        ...
                        if (
                            s &&
                            e &&
                            (instance.default_duration_minutes == null ||
                                instance.default_duration_minutes === 0)
                        ) {
                            // parse HH:MM(:SS)
                            const parseMin = (t) => {
                            ...
                            instance.default_duration_minutes = Math.max(
                                0,
                                diff
                            );
                        }
    ...
    CheckInType.associate = (models) => {
        // optional: will be referenced by CheckIn
        CheckInType.hasMany(models.Attendance, {
