...
export default (sequelize, DataTypes) => {
  const Attendance = sequelize.define(
    'Attendance',
    {
      ...
      // URL hình ảnh (text - 1 ảnh duy nhất cho mỗi bản ghi)
      photo_urls: {
        type: DataTypes.TEXT,
        allowNull: true,
      },
    ...
    },
    {
      ...
      hooks: {
        // Hook: compute duration_minutes, set status from check_out_time,
        // backfill photo_urls from photo_url, and populate location_point from lat/lng
      ...
      // Backfill photo_urls from legacy photo_url
          try {
            if (!checkIn.photo_urls && checkIn.photo_url) {
              checkIn.photo_urls = checkIn.photo_url;
            }
          } catch (e) {
            // ignore
          }
        ...
        afterCreate: async (checkIn, options) => {
          ...
          const session = await AttendanceSession.findByPk(checkIn.attendance_session_id, { transaction: options.transaction });
          if (!session) return;
          ...
          if (checkIn.check_out_time) {
            await session.update({ ended_at: checkIn.check_out_time, status: 'closed', check_out_id: checkIn.id }, { transaction: options.transaction });
          }
                ...
                await Attendance.create(
                  {
                    ...
                    notes: checkIn.notes ? `${checkIn.notes} [Co-technician of ${primaryTechnicianId}]` : `Co-technician of ${primaryTechnicianId}`,
        ...
        afterUpdate: async (checkIn, options) => {
          ...
          if (checkIn.attendance_session_id && checkIn.check_out_time) {
            await AttendanceSession.update({ ended_at: checkIn.check_out_time, status: 'closed', check_out_id: checkIn.id }, { where: { id: checkIn.attendance_session_id }, transaction: options.transaction });
            return;
          }

          // Otherwise, try to find an open session for this user/work and close it
          if (checkIn.check_out_time) {
            const session = await AttendanceSession.findOne({ where: { user_id: checkIn.user_id, work_id: checkIn.work_id, ended_at: null }, transaction: options.transaction });
            if (session) {
              await session.update({ ended_at: checkIn.check_out_time, status: 'closed', check_out_id: checkIn.id }, { transaction: options.transaction });
